...
export class Tournaments {
  ...
  private tournamentsReceiveUpdates = null;     // If true, will show TourneyUpdates messages in the Console (this is required in order to update the number of players and other states in the tournaments panel in real time
  ...
  constructor() {
    ...
    val = storage.get('tournaments-receive-updates');
    if(val === 'true')
      this.tournamentsReceiveUpdates = true;
    else if(val === 'false')
      this.tournamentsReceiveUpdates = false;
  ...
  public leaveTournamentsPane() {
    ...
    storage.remove('tournaments-receive-updates');
  ...
  public handleMessage(msg: string): boolean {
    ...
    // User has changed their TourneyUpdates td variable
    ...
    // td ObserveTourney must be set in order to receive Tourney Updates  
    ...
    match = msg.match(/^:mamer TOURNEY #(\d+) UPDATE: Tourney #\d+ has started!/m);
    if(match) {
      const id = +match[1];
      ...
      if(matchLastLine) {
        ...
        games.forEach(game => {          
          ...
          const obsGameStr = game.gameID && game.whiteName !== this.session.getUser() && game.blackName !== this.session.getUser()
              ? `  <a href="javascript:void(0)" onClick="sessionSend('obs ${game.result.slice(1)}')">Observe</a>` 
              : ''; 
          cell = row.insertCell();
  ...
  public addTournament(data: any, alert?: boolean) {
    ...
    if(!card || !card.length) { // No match so create new card
        ...
        <div class="card tournament-card" data-tournament-type="tournament">
          <div class="card-body d-flex">
            <div class="flex-grow-1" style="min-width: 0;">
              ...
              <div class="tournament-type me-1" style="white-space: pre-wrap;"></div>
              <div class="tournament-date me-1" style="white-space: pre-wrap;"></div>
              <div class="tournament-num-players me-1" style="white-space: pre-wrap;"></div>
      ...
      card.on('click', '.tournament-players-link', () => {
        if(awaiting.has('players-dialog')) // Players modal already displaying
          return;
    ...
    const winnersStr = tourney.winners
        ? `<span class="tournament-card-label">${ageInHours < 2 ? 'Winner' : 'Last Winner'}${tourney.winners.includes(',') ? 's' : ''}:</span>  ${tourney.winners}${ageInHours >= 2 ? '  <a class="tournament-standings-link" href="javascript:void(0)">(Standings)</a>' : ''}`
        : '';
    ...
    card.find('.tournament-standings-button').toggle((tourney.running && tourney.status === 'started') || (!!tourney.winners && ageInHours < 1));
  ...
  public addKoTH(data: any, alert?: boolean) {
    ...
    koth.title = `KoTH ${koth.type.slice(0, -1)}`; // Remove 'r' from end of title
  ...
  addTeamLeague(data: any) {
    ...
    if(!card.length) {
      card = $(`
        <div class="card tournament-card" data-tournament-type="teamleague">
          <div class="card-body d-flex">
            <div class="flex-grow-1" style="min-width: 0;">
              <div class="tl-title" style="font-weight: bold;"></div>
              <div class="mt-1 me-1 tl-date" style="white-space: pre-wrap;"></div>
              <div class="mt-1 tl-description me-3" style="white-space: pre-wrap;"></div>
            </div>
            <div class="d-flex" style="justify-content: end; align-items: center">
              <div class="btn-group-vertical" style="gap: 10px">
                <button type="button" class="btn btn-outline-secondary btn-md tl-interested" title="I'm Interested!" style="display: none; white-space: nowrap;">I'm Interested!</button>
                <button type="button" class="btn btn-outline-secondary btn-md tl-uninterested" title="Stop Interest" style="display: none; white-space: nowrap;">Remove Interest</button>
              </div>
            </div>
          </div>
        </div>
      ...
      card.find('.tl-interested, .tl-uninterested').on('click', (e) => {
        const data = card.data('tournament-data');
    ...
    card.find('.tl-title').text(tl.title);
    card.find('.tl-description').html(tl.description);  
    const whenStr = `<span class="tournament-card-label">When:</span>  ${tl.date}`;
    card.find('.tl-date').html(whenStr);  
    
    card.find('.tl-interested').toggle(!tl.interested);
    card.find('.tl-uninterested').toggle(!!tl.interested);
  }
