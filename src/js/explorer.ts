...
import { ByteReader, ByteWriter, ByteStreamReader } from './utils';
...
export class Explorer {
  ...
  private metadataUrl = 'https://fastly.jsdelivr.net/gh/dhirallin/freechessclub-openingexplorer/data/masters.oe'; 
  // Get the whole file(s) from the regular CDN.
  private dataUrl = 'https://cdn.jsdelivr.net/gh/dhirallin/freechessclub-openingexplorer/data/masters.oe';    
...
export class LichessExplorer {
  ...
  public async findPosition(fen: string): Promise<ExplorerPosition | undefined> {
    ...
    const moves = position.moves.map(entry => {
      const { move } = parseMove(fen, entry.san);
      if(!move) 
        return; 
      ...
      return {
        ...
        ...(entry.game && { lastYear: entry.game.year })
      }
    });   
