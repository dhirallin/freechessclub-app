...
export class Engine {
  protected static engineName: string; // The name of the engine currently loading/loaded
  ...
  public async init(game: Game, engineName?: string, options?: object) {
    ...
    this.multiThreaded = Engine.multiThreadedBlob;
  ...
  public static load(engineName?: string) {
    ...
    if(this.loadPromise && engineName === this.engineName) // load() has already been called
    ...
    this.loadPromise = (async () => {
      ...
      if(engineName === 'Stockfish MV 2019') {
        ...
        if(wasmSupported) {     
          this.multiThreadedBlob = hasMultiThreading();
