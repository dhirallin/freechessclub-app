...
export class Chat {
  ...
  public initStartChatMenu() {
    ...
    $('#start-chat-input').on('input', (event) => {
      ...
      else if(!this.userListRequested) {
        requestUserList();
        this.userListRequested = true; // Only request the user list once for each time the menu is shown
      }
    });
  ...
  private updateStartChatMenuFilter() {
    if($('#start-chat-menu').hasClass('show')) {
      ...
      if(inputText.length) {
        ...
        let matchingChannels = Object.entries(channels).filter(([key, value]) => value.toLowerCase().startsWith(inputText));
